...
async function handlePublish({ values, errors }: {
...
<template>
  <div class="publish-container">
    <a-layout>
      ...
      <a-layout-content>
        <a-form :model="form" layout="vertical" @submit="handlePublish">
          <a-form-item field="title" label="文章标题" :rules="[{ required: true, message: '请输入文章标题' }]">
            <a-input v-model="form.title" placeholder="请输入文章标题" />
          </a-form-item>
          <a-form-item field="desc" label="文章描述" :rules="[{ required: true, message: '请输入文章描述' }]">
            <a-input v-model="form.desc" placeholder="请输入文章描述" />
          </a-form-item>
          <a-form-item field="categories" label="文章分类" :rules="[{ required: true, message: '请选择文章分类' }]">
            <a-select v-model="form.categories" placeholder="请选择文章分类" multiple allow-create />
          </a-form-item>
            ...
            <a-row :gutter="16" style="height: 100%">
              ...
              <a-col :span="12">
                <a-form-item field="content" label="文章内容" :rules="[{ required: true, message: '请输入文章内容' }]">
                  <a-textarea v-model="form.content" placeholder="请输入文章内容" :auto-size="{ minRows: 10, maxRows: 20 }" />
                </a-form-item>
              </a-col>
